...
def finetuning_evaluate(model, test_dataloader, epoch, results_file_path, initial_hit, best_hit, eval_metric, best_rec,
                        path):
    hit_ft = [[], [], [], [], []]
    # Fine-tuning Test
    ...
    for k in range(len(topk)):
        ...
        if best_val < hit_score:
            best_val = hit_score
            best_rec[k] = best_val
            torch.save(model.state_dict, path[:-3] + '_' + str(topk[k]) + path[-3:])
...
def train_recommender(args, model, train_dataloader, test_dataloader, path, results_file_path, pretrain_dataloader):
    ...
    pretrain_evaluate(model, pretrain_dataloader, epoch, results_file_path, content_hit)
    finetuning_evaluate(model, test_dataloader, epoch, results_file_path, initial_hit, best_hit, eval_metric, best_rec,
                        path)
