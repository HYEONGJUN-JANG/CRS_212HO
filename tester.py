...
if __name__ == '__main__':
    ...
    with open(results_file_path, 'w', encoding='utf-8') as result_f:
        ...
        for t in range(NUM_TRIAL):
            ...
            result_f.write('content_hits:\t' + '\t'.join(format(x, ".2f") for x in content_hits[t]) + '\n')
            result_f.write('initial_hits:\t' + '\t'.join(format(x, ".2f") for x in initial_hits[t]) + '\n')
            result_f.write('best_hits:\t' + '\t'.join(format(x, ".2f") for x in best_results[t]) + '\n')
