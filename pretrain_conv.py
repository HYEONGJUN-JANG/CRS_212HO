...
def evaluate(titles, response, preds, tokenizer, log=False, log_file_path=None):
    ...
    decoded_responses = [decoded_response.replace('[PAD]', '').replace('[SEP]', '') for decoded_response in
                         decoded_responses]
    ...
    decoded_titles = [decoded_title.replace('[PAD]', '').replace('[SEP]', '') for decoded_title in
                      decoded_titles]
...
def pretrain_conv(args, model, gpt_model, gpt_config, tokenizer, pretrain_dataloader, pretrain_dataloader_test,
    ...
    for epoch in range(args.conv_epoch_pt):
        ...
        for batch in tqdm(pretrain_dataloader, bar_format=' {percentage:3.0f} % | {bar:23} {r_bar}'):
            ...
            loss = gpt_model(**batch['context'], conv_labels=batch['response'], encoder_hidden_states=token_embedding,
                             encoder_attention_mask=token_padding_mask, conv=True).conv_loss
